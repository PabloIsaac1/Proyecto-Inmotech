...
const ViewAppointmentModal = ({ isOpen, onClose, cita }) => {
  ...
  const formatTime = (timeString) => {
    ...
    // Step 1: Clean multiple AM/PM suffixes (case-insensitive)
    let cleanedTime = timeString.trim();

    // Find all AM/PM matches (case-insensitive)
    const ampmMatches = cleanedTime.match(/\b(am|pm)\b/gi) || [];
    const totalSuffixes = ampmMatches.length;

    if (totalSuffixes > 1) {
      // Remove all AM/PM suffixes
      cleanedTime = cleanedTime.replace(/\s*\b(am|pm)\b/gi, '').trim();

      // Add back only the last suffix (lowercase)
      const lastSuffix = ampmMatches[totalSuffixes - 1].toLowerCase();
      cleanedTime += ' ' + lastSuffix;
    }

    // Step 2: Check if already has AM/PM (case-insensitive)
    if (/\b(am|pm)\b/i.test(cleanedTime)) {
      return cleanedTime;
    }

    // Step 3: Convert from 24-hour format to 12-hour format
    const timeParts = cleanedTime.split(':');
    if (timeParts.length !== 2) return cleanedTime; // Invalid format, return as-is

    const hours24 = parseInt(timeParts[0], 10);
    const minutes = timeParts[1];

    if (isNaN(hours24)) return cleanedTime; // Invalid hours, return as-is
